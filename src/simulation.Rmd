...
observations <- participants %>% 
  ...
  mutate(
    period = factor(period, levels=1:2, ordered=TRUE),
    group = factor(group, levels=1:2, ordered=FALSE)
  )
```
...
f5 <- response ~ 1 + treatment*period + (1|group/PID)

get_prior(f5, data=observations, family=gaussian)
```
...
```{r model5}
m5 <-
  brm(data = observations, family = gaussian, f5, prior = priors2,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4,
    #file = "fits/m5"
  )
...
```{r}
brms::pp_check(m5, ndraws=ndraws)
```
...
m5.coefs.fixed <- data.frame(fixef(m5, summary = TRUE))
m5.coefs.fixed <- cbind(factor = rownames(m5.coefs.fixed), m5.coefs.fixed)
m5.coefs.fixed$model <- 'm5'

coefs.fixed <- rbind(m1.coefs.fixed, m2.coefs.fixed, m3.coefs.fixed, m4.coefs.fixed, m5.coefs.fixed)
...
d.effects.coefs <- cbind(factor = c(
  'Intercept', 'treatmentTRUE', 'PID', 'treatmentTRUE:period.L', 'treatmentFALSE:period.L', 'period.L'
  ), d.effects) %>% 
...
position.offset <- position_nudge(
  y = ifelse(coefs.fixed$model == "m1", 0.3,
             ...
             ifelse(coefs.fixed$model == "m3", -0.1, 
              ifelse(coefs.fixed$model == "m4", -0.3, -0.5))))
)
...
coefs.fixed %>% ggplot(aes(y = factor, color = model)) +
  ...
  scale_y_discrete(limits=rev(c("Intercept", "treatmentTRUE", "period.L", "treatmentTRUE:period.L")))
```
